...
```powershell
python scripts\console.py --config configs/train_shakespeare_char.py --ckpt outputs/gpt_shakespeare_char_best.pt --temperature 0.8 --top-k 20 --max-new-tokens 200 --log results/console_shakespeare_char.log
```
...
```powershell
python scripts\console.py --config configs/train_shakespeare_char.py --ckpt outputs/gpt_shakespeare_char_best.pt --temperature 0.8 --top-k 20 --max-new-tokens 200
```

保存交互日志：

```powershell
python scripts\console.py --config configs/train_shakespeare_char.py --ckpt outputs/gpt_shakespeare_char_best.pt --log results/console_shakespeare_char.log
```
...
| 评估与实验对比 | 基本完成 | 建立统一评估入口，支持不同 tokenizer 与 checkpoint 的量化比较 | `scripts/eval.py`、validation loss、perplexity、char vs byte 对比表、README 实验结果更新 |
| 交互生成增强 | 基本完成 | 提升模型演示和人工观察效率 | `scripts/console.py --ckpt`、生成参数展示、生成日志保存、CLI smoke test |
