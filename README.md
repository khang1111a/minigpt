...
* 支持交互式文本生成控制台，可指定 checkpoint 并保存生成日志。
* 支持 loss 曲线可视化。
...
```powershell
python -m minigpt prepare --input data/tiny_text/input.txt --out data/tiny_text_char --tokenizer char
```

### 2. 训练字符级 GPT 模型

```powershell
python -m minigpt train --config configs/train_char.py
```

### 3. 使用 prompt 生成文本

```powershell
python -m minigpt sample --config configs/train_char.py --prompt "this is" --temperature 0.8 --top-k 10 --max-new-tokens 200
```

### 4. 绘制训练损失曲线

```powershell
python -m minigpt plot --loss results/loss_char.csv
```
...
```powershell
python -m minigpt train --config configs/train_shakespeare_char.py
python -m minigpt train --config configs/train_shakespeare_byte.py
```

训练完成后可以生成文本：

```powershell
python -m minigpt sample --config configs/train_shakespeare_char.py --prompt "ROMEO:" --temperature 0.8 --top-k 20 --max-new-tokens 500 --seed 42
python -m minigpt sample --config configs/train_shakespeare_byte.py --prompt "ROMEO:" --temperature 0.8 --top-k 40 --max-new-tokens 500 --seed 42
```

也可以显式加载 best checkpoint：

```powershell
python -m minigpt sample --config configs/train_shakespeare_char.py --ckpt outputs/gpt_shakespeare_char_best.pt --prompt "ROMEO:" --temperature 0.8 --top-k 20 --max-new-tokens 500 --seed 42
```

评估 best checkpoint：

```powershell
python -m minigpt eval --config configs/train_shakespeare_char.py --ckpt outputs/gpt_shakespeare_char_best.pt --eval-iters 20
python -m minigpt eval --config configs/train_shakespeare_byte.py --ckpt outputs/gpt_shakespeare_byte_best.pt --eval-iters 20
```

启动交互式生成控制台：

```powershell
python -m minigpt console --config configs/train_shakespeare_char.py --ckpt outputs/gpt_shakespeare_char_best.pt --temperature 0.8 --top-k 20 --max-new-tokens 200 --log results/console_shakespeare_char.log
```
...
如果已经执行过 `python -m pip install -e .`，也可以直接使用：

```powershell
minigpt --help
```

当前 CLI 兼容并复用 `scripts/` 下的实现。`scripts/*.py` 仍然保留，方便调试和向后兼容。
...
```powershell
python -m minigpt prepare --input data/tiny_text/input.txt --out data/tiny_text_byte --tokenizer byte
```
...
```powershell
python -m minigpt prepare --input data/shakespeare/input.txt --out data/shakespeare_char --tokenizer char --train-ratio 0.9
```

Byte-level 数据：

```powershell
python -m minigpt prepare --input data/shakespeare/input.txt --out data/shakespeare_byte --tokenizer byte --train-ratio 0.9
```
...
早期脚本 `data/tiny_text/prepare.py` 保留用于兼容。新的实验建议统一使用 `python -m minigpt prepare`。
...
```powershell
python -m minigpt train --config configs/train_byte.py
```
...
```powershell
python -m minigpt train --config configs/train_shakespeare_char.py --resume outputs/gpt_shakespeare_char.pt
```
...
```text
python -m minigpt sample
```

字符级生成：

```powershell
python -m minigpt sample --config configs/train_char.py --prompt "this is" --max-new-tokens 200
```

带采样控制的生成：

```powershell
python -m minigpt sample --config configs/train_char.py --prompt "this is" --temperature 0.8 --top-k 10 --seed 1337
```

Byte-level 生成：

```powershell
python -m minigpt sample --config configs/train_byte.py --prompt "this is" --temperature 0.7 --top-k 50
```
...
```text
python -m minigpt console
```

控制台用于持续输入 prompt 并观察模型续写结果，适合人工检查 checkpoint 的生成风格和采样参数影响。

使用 Shakespeare 字符级 best checkpoint 启动控制台：

```powershell
python -m minigpt console --config configs/train_shakespeare_char.py --ckpt outputs/gpt_shakespeare_char_best.pt --temperature 0.8 --top-k 20 --max-new-tokens 200
```

保存交互日志：

```powershell
python -m minigpt console --config configs/train_shakespeare_char.py --ckpt outputs/gpt_shakespeare_char_best.pt --log results/console_shakespeare_char.log
```
...
:topk <value>      set top_k
:topk none         disable top_k
:max <value>       set max_new_tokens
:show              show current generation settings
```

`--log` 会记录 config、checkpoint、生成参数、用户输入和模型输出。生成日志属于实验产物，建议按需保存在 `results/` 下，不作为默认提交内容。

## 模型评估

推荐使用统一 CLI 评估 checkpoint：

```text
python -m minigpt eval
```

评估命令用于独立评估指定 checkpoint，不依赖训练过程中的日志。它会读取配置文件中的数据集与评估参数，读取 checkpoint 中的模型结构和权重，并输出 loss 与 perplexity。
...
```powershell
python -m minigpt plot --loss results/loss_byte.csv
```

自定义输出路径：

```powershell
python -m minigpt plot --loss results/loss_byte.csv --out results/byte_curve.png
```

绘制 Shakespeare 实验曲线：

```powershell
python -m minigpt plot --loss results/loss_shakespeare_char.csv --out results/loss_shakespeare_char.png
python -m minigpt plot --loss results/loss_shakespeare_byte.csv --out results/loss_shakespeare_byte.png
```
...
`tiny_text` 的验证集太短，主要用于快速检查 tokenizer、dataset、model、train、sample 是否能跑通。Shakespeare 实验使用相同模型规模训练 500 step，每 100 step 记录一次 train / validation loss，并通过 `python -m minigpt eval` 对 best checkpoint 进行独立评估；当前结果说明 char 与 byte 两条 tokenizer 管线都能完整训练、恢复、评估、生成和绘图。
...
| 评估与实验对比 | 基本完成 | 建立统一评估入口，支持不同 tokenizer 与 checkpoint 的量化比较 | `python -m minigpt eval`、validation loss、perplexity、char vs byte 对比表、README 实验结果更新 |
| 交互生成增强 | 基本完成 | 提升模型演示和人工观察效率 | `python -m minigpt console --ckpt`、生成参数展示、生成日志保存、CLI smoke test |
...
```text
1. 为 eval.py 增加 CSV / JSON 输出，便于后续批量实验汇总。
2. 推进 Simple BPE tokenizer，补齐 char / byte / BPE 对比链路。
3. 将 console 生成日志纳入实验样例整理流程。
4. 在 tokenizer 与评估链路稳定后，推进 LoRA / Adaptive LoRA。
```
...
```powershell
python -m minigpt sample --config configs/train_byte.py --temperature 0.7 --top-k 50
```
